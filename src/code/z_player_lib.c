...
s16 sBootData[PLAYER_BOOTS_MAX][17] = {
    ...
    { 200, 1000, 300, 700, 550, 270, 1000, 0, 800, 300, -160, 600, 590, 750, 125, 200, 130 }, // Iron Boots Adult
    { 200, 1000, 300, 700, 550, 270, 600, 600, 800, 550, -100, 600, 540, 270, 25, 0, 130 }, // Hover Boots Adult
    { 200, 1000, 300, 700, 380, 400, 0, 300, 800, 500, -100, 600, 590, 750, 125, 200, 130 }, // Indoor
    { 80, 800, 150, 700, 480, 270, 600, 50, 800, 550, -40, 400, 540, 270, 25, 0, 80 }, // Underwater
    { 200, 1000, 300, 800, 700, 400, 800, 400, 800, 1000, -100, 600, 540, 750, 125, 400, 200 }, // Kokiri Boots Child (Wolf Properties)
};
...
// TODO: Come back to this; not right for child
    if (!Player_OverrideLimbDrawGameplayCommon(play, limbIndex, dList, pos, rot, thisx)) {
        ...
        } else if (limbIndex == PLAYER_ADULT_LIMB_L_HAND) {
            *dList = sFirstPersonLeftHandDLs[(void)0, gSaveContext.save.linkAge];
        } else if (limbIndex == PLAYER_ADULT_LIMB_R_SHOULDER) {
            *dList = sFirstPersonRightShoulderDLs[(void)0, gSaveContext.save.linkAge];
        } else if (limbIndex == PLAYER_ADULT_LIMB_R_FOREARM) {
...
// seems to process each limb during gameplay
